  ...
  return (
    <div className="min-h-screen pb-10 space-y-6">
      ...
      <div className="max-w-[1600px] mx-auto px-3 sm:px-4 lg:px-5 pt-4">
        <button
          onClick={() => navigate(-1)}
          className="inline-flex items-center gap-2 text-xs font-mono text-monochrome-500 hover:text-brand font-semibold transition-colors"
        >
      ...
      <div className="max-w-[1600px] mx-auto px-3 sm:px-4 lg:px-5">
        <div className="relative h-[380px] md:h-[480px] rounded-3xl overflow-hidden border border-monochrome-800 shadow-2xl">
          ...
          <div className="absolute top-6 left-6 flex items-center gap-3">
            <span className="px-3.5 py-1.5 rounded-full text-xs font-bold bg-white/95 backdrop-blur-md text-brand border border-brand/10 uppercase shadow-sm">
              {event.category}
            </span>
            <span className={`px-3.5 py-1.5 rounded-full text-xs font-bold font-mono border ${
              isFree 
      ...
      <div className="max-w-[1600px] mx-auto px-3 sm:px-4 lg:px-5 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <section className="glass-panel p-8 rounded-3xl space-y-4">
            <h3 className="text-xl font-extrabold text-monochrome-200 tracking-tight">About This Event</h3>
            <p className="text-monochrome-350 text-sm leading-relaxed whitespace-pre-line">
              {event.description}
            </p>
          </section>
          ...
          {event.speakers && event.speakers.length > 0 && (
            <section className="space-y-4">
              <h3 className="text-xl font-extrabold text-monochrome-250 tracking-tight">Keynote Speakers</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {event.speakers.map((sp, idx) => (
                  <div key={idx} className="glass-panel p-5 rounded-2xl flex items-center gap-4 border border-monochrome-700">
                    <img
                    ...
                    />
                    <div>
                      <h4 className="font-bold text-monochrome-100 text-sm">{sp.name}</h4>
                      <p className="text-xs text-monochrome-500 font-mono">{sp.title}</p>
                      {sp.bio && <p className="text-[11px] text-monochrome-555 mt-1 line-clamp-2">{sp.bio}</p>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          {event.schedule && event.schedule.length > 0 && (
            <section className="space-y-4">
              <h3 className="text-xl font-extrabold text-monochrome-255 tracking-tight">Agenda & Schedule</h3>
              <div className="glass-panel p-6 rounded-3xl divide-y divide-monochrome-700">
                {event.schedule.map((item, idx) => (
                  <div key={idx} className="py-4 first:pt-0 last:pb-0 flex items-start gap-4">
                    <div className="w-24 font-mono text-xs font-bold text-monochrome-200 flex items-center gap-1.5 flex-shrink-0 pt-0.5">
                      <Clock className="w-3.5 h-3.5 text-brand" />
                      {item.time}
                    </div>
                    <div>
                      <h5 className="text-sm font-bold text-monochrome-100">{item.topic}</h5>
                      {item.speaker && <p className="text-xs text-monochrome-500 mt-0.5">Speaker: {item.speaker}</p>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <section className="glass-panel p-8 rounded-3xl space-y-4">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-xl font-extrabold text-monochrome-200 tracking-tight">Venue & Location</h3>
                <p className="text-monochrome-100 font-bold text-sm mt-1">{venueName}</p>
                <p className="text-xs text-monochrome-500">{address}, {city}, India</p>
              </div>

              <span className="text-[10px] font-mono bg-monochrome-800 border border-monochrome-700 text-monochrome-400 px-2.5 py-1 rounded">
        ...
        <div className="space-y-6">
          <div className="sticky top-24 glass-card p-6 rounded-3xl border border-monochrome-700 shadow-2xl space-y-6">
            ...
            <div className="border-b border-monochrome-750 pb-4">
              <span className="text-[10px] uppercase font-mono tracking-widest text-monochrome-500">TICKET RESERVATION</span>
              <div className="flex items-baseline justify-between mt-1">
                <span className="text-3xl font-extrabold font-mono text-monochrome-100">
            ...
            <div className="p-4 rounded-xl bg-monochrome-800 border border-monochrome-700 space-y-2">
              <div className="flex justify-between items-center text-xs">
                <span className="text-monochrome-500">Remaining Capacity</span>
                <span className="font-mono font-bold text-monochrome-100">
              ...
              <div className="w-full h-2 rounded-full bg-monochrome-950 overflow-hidden">
                <div
                  className="h-full bg-brand shadow shadow-brand/20 transition-all duration-500"
            ...
            {/* Organizer Info */}
            <div className="p-4 rounded-xl bg-monochrome-800/50 border border-monochrome-700 flex items-center gap-3">
              <User className="w-5 h-5 text-brand" />
              <div>
                <span className="text-[10px] text-monochrome-500 font-mono block">Hosted by</span>
                <span className="text-xs font-bold text-monochrome-100">{event.organizerName || 'Event Host'}</span>
              </div>
            </div>
            ...
            {isSoldOut ? (
              <button
                disabled
                className="w-full py-3.5 rounded-xl bg-monochrome-800 text-red-500 font-bold text-xs border border-monochrome-700 cursor-not-allowed"
              >
              ...
              <button
                ...
                className="w-full py-3.5 rounded-xl bg-brand text-white font-extrabold text-xs hover:bg-brand-hover transition-all shadow-lg shadow-brand/10 flex items-center justify-center gap-2"
              >
